package ru.mpei.brics.agentDetector.utils;
...
public class PcapHelper {
     ...
     * @param T - time period to get packets from network card buffer in ms
     */
    ...
    private ScheduledFuture<?> startCapturingTask(int port, PacketListener pl, ScheduledExecutorService ses, PcapHandle pcapHandle) {
        return ses.schedule(() -> {
            try {
                ...
                log.info("The interface has been successfully applied");
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private PcapHandle openIFaceToCapturePackets(String ifaceName, int T) {
        ...
        if (chosenIface == null) {
            log.error("Failed to find iface with name{}  ethernet available interface are : {}",
                    ifaceName, foundIfaces.stream().map(el -> el.getName() + el.getAddresses()).collect(Collectors.toList()));
